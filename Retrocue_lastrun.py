...
This experiment was created using PsychoPy3 Experiment Builder (v2020.2.10),
    on Wed 10 Mar 2021 01:19:49 PM CET
...
expInfo = {'participant': '', 'session': '001', 'age': '', 'sex at birth': '', 'dominant hand': '', 'instructions': '1', 'realRuns': '0'}
...
# Setup the Window
win = visual.Window(
    size=[1536, 801], fullscr=True, screen=0, 
...
probeInst = visual.TextStim(win=win, name='probeInst',
    text='You just saw the test image. In this case, it was part of the memorized images. Press the correct button to continue. You can answer immediately after the test image is presented without waiting for the green fixation cross. ',
    font='Arial',
...
Button_reminder = visual.TextStim(win=win, name='Button_reminder',
    text='Response reminder: left button test image was part of memory images. \nRight button if test image was NOT part of memory images\n(Press a button to start practice trials)',
    font='Arial',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
txt_final = visual.TextStim(win=win, name='txt_final',
    text='Congratulations! You have finished the experiment. ',
    font='Arial',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    ...
    for thisRepeat_InstrTrial in repeat_InstrTrials:
        ...
        for thisTrials_instr2 in trials_instr2:
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if probe_Practice.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if fixProbe_Practice.status == NOT_STARTED and tThisFlip >= 14.7-frameTolerance:
                    # keep track of start time/frame for later
                ...
                waitOnFlip = False
                if keyResp_Practice.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                    # keep track of start time/frame for later
            ...
            percent_correct = str(round(number_correct/trials_instr2.nTotal*100,2))
            thisExp.nextEntry()
...
# get names of stimulus parameters
if dontSkipInstructions.trialList in ([], [None], None):
    params = []
else:
    params = dontSkipInstructions.trialList[0].keys()
# save data for this loop
dontSkipInstructions.saveAsText(filename + 'dontSkipInstructions.csv', delim=',',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
...
for thisRunReal in runReal:
    ...
    for thisRun in runs:
        ...
        if trigger1.keys in ['', [], None]:  # No response was made
            trigger1.keys = None
        runs.addData('trigger1.keys',trigger1.keys)
        if trigger1.keys != None:  # we had a response
            runs.addData('trigger1.rt', trigger1.rt)
        runs.addData('trigger1.started', trigger1.tStartRefresh)
        runs.addData('trigger1.stopped', trigger1.tStopRefresh)
        ...
        for thisTrial in trials:
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                # *probe* updates
                if probe.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                    # keep track of start time/frame for later
                ...
                # *fixProbe* updates
                if fixProbe.status == NOT_STARTED and tThisFlip >= 14.7-frameTolerance:
                    # keep track of start time/frame for later
                ...
                # *keyResp* updates
                waitOnFlip = False
                if keyResp.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                    # keep track of start time/frame for later
            ...
            percent_correct = str(round(number_correct/trials.nTotal*100,2))
            thisExp.nextEntry()
        ...
        space_Run.rt = []
        _space_Run_allKeys = []
        feedbackTxtRun.setText('Your performance was '+percent_correct+'%.')
        # keep track of which components have finished
    ...
    while continueRoutine:
        ...
        if space_final.status == STARTED and not waitOnFlip:
            theseKeys = space_final.getKeys(keyList=['b', 'y'], waitRelease=False)
    ...
    runReal.addData('space_final.keys',space_final.keys)
    if space_final.keys != None:  # we had a response
        runReal.addData('space_final.rt', space_final.rt)
    runReal.addData('space_final.started', space_final.tStartRefresh)
    runReal.addData('space_final.stopped', space_final.tStopRefresh)
    runReal.addData('txt_final.started', txt_final.tStartRefresh)
    runReal.addData('txt_final.stopped', txt_final.tStopRefresh)
...
# get names of stimulus parameters
if runReal.trialList in ([], [None], None):
    params = []
else:
    params = runReal.trialList[0].keys()
# save data for this loop
runReal.saveAsText(filename + 'runReal.csv', delim=',',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
