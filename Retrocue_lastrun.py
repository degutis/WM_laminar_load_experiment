...
trials_Instr = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('trials_Int.xlsx', selection='0'),
...
for thisTrials_Instr in trials_Instr:
    ...
    while continueRoutine:
        ...
        if reponseInstr.status == STARTED and not waitOnFlip:
            theseKeys = reponseInstr.getKeys(keyList=['left'], waitRelease=False)
...
for thisRepeat_InstrTrial in repeat_InstrTrials:
    ...
    trials_instr2 = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('trials_Int.xlsx', selection='1:9'),
    ...
    for thisTrials_instr2 in trials_instr2:
        ...
        # keep track of which components have finished
        trial_practiceComponents = [stim1_Practice, stim2_Practice, stim3_Practice, stim4_Practice, fixDelay_Practice, probe_Practice, probeIndicator_Practice, fixProbe_Practice, keyResp_Practice, mask1_prac, mask2_prac, mask3_prac, mask4_prac]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if probeIndicator_Practice.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if probeIndicator_Practice.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > probeIndicator_Practice.tStartRefresh + 1.2-frameTolerance:
                    # keep track of stop time/frame for later
...
for thisRun in runs:
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        trialComponents = [stim1, stim2, stim3, stim4, fixDelay, probe, probeIndicator, fixProbe, keyResp, mask1_run, mask2_prac_2, mask3_run, mask4_run]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if probeIndicator.status == NOT_STARTED and tThisFlip >= 13.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if probeIndicator.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > probeIndicator.tStartRefresh + 1.2-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        trials.addData('probeIndicator.started', probeIndicator.tStartRefresh)
        trials.addData('probeIndicator.stopped', probeIndicator.tStopRefresh)
